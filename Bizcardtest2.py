...
icon = Image.open("C:/Users/ajayc/Downloads/businesscard.jpg")
...
st.write('[  Business card scanning and recognition with OCR engine ,this project enables your end users to scan and recognize the business cards with ease and high accuracy. ]')



# CREATING OPTION MENU
selected = option_menu(None, ["Home Page", "Upload & Modification", "Delete Details"],
...
if selected == "Home Page":
    ...
    with col1:
        st.image(Image.open("C:/Users/ajayc/Downloads/businesscard.jpg"), width=500)
        st.write(
            "## :green[**Process of Extraction**]")
        st.header(":blue[Extracting Card Details :]")
    ...
    with col2:
        st.write(
            "## :green[**About the Project :**] Bizcard is a Python application designed to extract information from business cards with OCR (Optical Character Recognition)")
        st.header(":blue[Technologies used :]")
        st.subheader("Business Card Details Extraction")
        st.write("   **-> Python**")
        st.write("   **-> EasyOCR**")
        st.write("   **->Streamlit**")
        st.write("   **->SQL**")
        st.write("   **->Pandas**")
        st.image(Image.open("C:/Users/ajayc/Downloads/businesscard2.jpg"), width=500)
...
if selected == "Delete Details":
    ...
    with col_b:
        remove = st.button("Click to delete details")
...
if selected == "Upload & Modification":
    ...
    if image is not None:
        ...
        if selected == "Preview":
            ...
            with col3:
                Preview = st.button("Preview modified details")
            with col4:
                Upload = st.button("Upload details")
            if Preview:
